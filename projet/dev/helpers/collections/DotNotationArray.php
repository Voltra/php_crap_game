...
class DotNotationArray implements ArrayAccess {
     ...
     */
    public static function makeFrom(iterable $arr){
     ...
     * @return DotNotationArray
     */
    public static function newFrom(iterable $arr){
     ...
     * @return bool
     */
    protected static function exists(iterable $array, string $key): bool {
        if(isset($array[$key]))
            return true;
     ...
     * @return mixed
     */
    protected static function get(iterable $array, string $key) {
        if(isset($array[$key]))
            return $array[$key];
     ...
     */
    protected static function set(iterable& $array, string $key, $value) {
        $keys = explode(".", $key);
        while(count($keys) > 1){
     ...
     */
    protected static function unset(iterable& $array, string $key) {
        $keys = explode(".", $key);
        while(count($keys) > 1){
            $curr_key = array_shift($keys);
